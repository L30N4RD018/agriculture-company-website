...
import AddUserForm from "@/app/ui/dashboard/AddUsersForm"
...
export default function UsersTable(props: TableProps) {
    ...
    const handleDelete = async () => {
        ...
        if (confirm(`¿Estás seguro de eliminar ${selectedItems.length} elementos?`)) {
            await Promise.all(selectedItems.map(item => deleteInfo(item.id, 'users', (message) => {errorToast(message)}, (message) => {successToast(message)})));
    ...
    const handleSubmit = async (data: {[key: string]: any}) => {
        ...
        await createUpdate({data, update: false, formKey: 'users'}, (message) => {errorToast(message)}, (message) => {successToast(message)});
    }

    useEffect(() => {
        fetchTableInfo('users', (message) => {errorToast(message)}).then((data) => {
            setData(data);
        });
    }, [isModalOpen]);
    ...
    return (
        <div className="flex flex-col w-full gap-1">
            <div className="flex h-16 w-full items-center px-2 gap-4">
                <div className="w-full"><h2 className="text-lg font-bold">Gestión de Usuarios</h2></div>
                {
                    session?.user?.permissions.includes('create:users') && (
                ...
                {isModalOpen && <AddUserForm onClose={handleClose} onSubmit={handleSubmit}/>}
                {
                    session?.user?.permissions.includes('delete:users') && (
                    ...
                    <tbody>
                        {currentItems.map((item, index) => (
                            <tr key={item.id} className="text-xs border-b border-gray-300 text-center max-w-10">
                                ...
                                <td>{item.name}</td>
                                <td>
                                    <select className="form-select bg-white border border-gray-300 rounded-md text-xs p-1">
                                ...
                                <td className="flex gap-1 justify-center">                                    
                                    ...
                                    {
                                        session?.user?.permissions.includes('update:users') && (
                                            <button className="flex items-center justify-center p-1 rounded-md w-8 hover:bg-blue-300">
