...
import AddSmallholdingForm from "@/app/ui/dashboard/AddSmallholdingsForm"
...
export default function smallholdingsTable(props: TableProps) {
    ...
    const handleDelete = async () => {
        ...
        if (confirm(`¿Estás seguro de eliminar ${selectedItems.length} elementos?`)) {
            await Promise.all(selectedItems.map(item => deleteInfo(item.id, 'smallholdings', (message) => {errorToast(message)}, (message) => {successToast(message)})));
    ...
    const handleSubmit = async (data: {[key: string]: any}) => {
        ...
        await createUpdate({data, update: false, formKey: 'smallholdings'}, (message) => {errorToast(message)}, (message) => {successToast(message)});
    }

    useEffect(() => {
        fetchTableInfo('smallholdings', (message) => {errorToast(message)}).then((data) => {
            setData(data);
        });
    }, [isModalOpen]);
    ...
    return (
        <div className="flex flex-col w-full gap-1">
            <div className="flex h-16 w-full items-center px-2 gap-4">
                <div className="w-full"><h2 className="text-lg font-bold">Gestión de Parcelas</h2></div>
                {
                    session?.user?.permissions.includes('create:smallholdings') && (
                ...
                {isModalOpen && <AddSmallholdingForm onClose={handleClose} onSubmit={handleSubmit}/>}
                {
                    session?.user?.permissions.includes('delete:smallholdings') && (
                        ...
                        {currentItems.map((item, index) => (
                            <tr key={item.id} className="text-xs border-b border-gray-300 text-center max-w-10">
                                ...
                                <td className="flex gap-1 justify-center">                                    
                                    ...
                                    {
                                        session?.user?.permissions.includes('update:smallholdings') && (
                                            <button className="flex items-center justify-center p-1 rounded-md w-8 hover:bg-blue-300">
